...
from models.glorot_normal_scaled_initializer import GlorotNormalScaled

tf1, tf, tfv = try_import_tf()

class FullyConnectedNetwork_GlorotNormalInitializer(TFModelV2):
    """ A fully connected Network - same as the provided generic one
        (ray.rllib.models.tf.FullyConnectedNetwork), but using the
        Glorot Normal initialization (scaled for action output which
        requires a derived scaled initializer).
    """

    def __init__(self, obs_space, action_space, num_outputs, model_config,
                 name):
        super(FullyConnectedNetwork_GlorotNormalInitializer, self).__init__(
        ...
        for size in hiddens[:-1]:
            last_layer = tf.keras.layers.Dense(
                ...
                kernel_initializer=GlorotNormalScaled(1.0))(last_layer)
            i += 1
        ...
        # iff num_outputs > 0 - a last linear layer of size num_outputs.
            ...
            if num_outputs:
                logits_out = tf.keras.layers.Dense(
                    ...
                    name="fc_out",
                    activation=None,
                    kernel_initializer=GlorotNormalScaled(0.01))(last_layer)
        ...
        if not vf_share_layers:
            ...
            for size in hiddens:
                last_vf_layer = tf.keras.layers.Dense(
                    ...
                    activation=activation,
                    kernel_initializer=GlorotNormalScaled(1.0))(last_vf_layer)
                i += 1
        ...
        value_out = tf.keras.layers.Dense(
            ...
            name="value_out",
            activation=None,
            kernel_initializer=GlorotNormalScaled(0.01))(
